...
export default function Balances() {
  ...
  const handleClose = () => {
    setForm({ type: '', bank: '', bankLogo: '', number: '', amount: '' });
    setOpen(false);
  };
  ...
  const handleAddAccount = () => {
    if (form.type && form.bank && form.number && form.amount !== '') {
      const newAccount = {
        id: Date.now(),
        ...form,
  ...
  const [accountTypes, setAccountTypes] = useState(initialTypes);
  const [bankNames, setBankNames] = useState(initialBanks);
        ...
        <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
          ...
          >
            ...
            <Autocomplete
              ...
              value={form.type}
              onChange={(e, newValue) => {
                if (newValue && !accountTypes.includes(newValue)) {
                  setAccountTypes((prev) => [...prev, newValue]);
                }
            ...
            <Autocomplete
              ...
              value={form.bank}
              onChange={(e, newValue) => {
                if (newValue && !bankNames.includes(newValue)) {
                  setBankNames((prev) => [...prev, newValue]);
                }
                setForm((prev) => ({ ...prev, bank: newValue || '' }));
              }}
              renderInput={(params) => (
          ...
          <DialogActions>
            <Button
              onClick={handleClose}
              sx={{ width: '160px', height: '48px', borderRadius: '6px' }}
            >
              Cancel
            </Button>
            <Button
